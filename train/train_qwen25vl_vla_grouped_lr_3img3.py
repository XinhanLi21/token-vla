...
"""
train_qwen25vl_vla_grouped_lr_3img2.py

Same as your grouped-lr 3-image VLA trainer, but **restrict to 2 GPUs** by
setting CUDA_VISIBLE_DEVICES before importing torch/transformers.

Default: use GPU 0 and 1 (CUDA_VISIBLE_DEVICES="0,1").

Usage:
  python train_qwen25vl_vla_grouped_lr_3img2.py ... --cuda_visible_devices "0,1"
"""
...
def _early_parse_cuda_visible_devices() -> str:
    ...
    ap.add_argument("--cuda_visible_devices", type=str, default="0,1",
                    help='Comma-separated GPU ids, e.g. "0,1". Default uses two GPUs.')
    args, _ = ap.parse_known_args()
    return args.cuda_visible_devices

_cuda_vis = _early_parse_cuda_visible_devices()
os.environ["CUDA_VISIBLE_DEVICES"] = _cuda_vis  # must be set before torch import
...
class DataCollatorQwenVLA3ImgTokens:
    ...
    def __call__(self, batch: List[Dict[str, Any]]) -> Dict[str, torch.Tensor]:
        ...
        for ex in batch:
            ...
            # order: middle, left, right (and text says same)
...
class GroupedLRTrainer(Trainer):
    ...
    # ✅ transformers新版: training_step(model, inputs, num_items_in_batch)
...
def main():
    ...
    # Remind what GPUs are visible to this process
    ...
    # device_map="auto" will ONLY see the GPUs in CUDA_VISIBLE_DEVICES
    ...
    targs = TrainingArguments(
        ...
        learning_rate=args.lr_base,     # placeholder; real lrs come from grouped optimizer
