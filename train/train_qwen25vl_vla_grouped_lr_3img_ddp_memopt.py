            ...
            for line in tqdm(f, desc="Scan jsonl"):
                ...
                try:
                    ...
                    if (not isinstance(tp, list)) or (not isinstance(tf, list)):
                        raise ValueError("bad_tokens_type")
                    if len(tp) != self.past_len or len(tf) != self.future_len:
                        raise ValueError("bad_tokens_len")
...
# -------------------------
# Collator: 3 images + prompt/state/past tokens -> labels on future tokens
# -------------------------
@dataclass
class DataCollatorQwenVLA3ImgTokens:
    ...
    token_ctx_key: str
    prefix: str = "traj"
    max_length: int = 1536          # ✅ 默认更安全，避免2048直接炸
...
def main():
    ...
    ap.add_argument("--max_length", type=int, default=1536)   # ✅ 默认更稳
    ...
    ap.add_argument("--deepspeed", type=str, default=None,
                    help="Path to deepspeed config json. If set, Trainer will use ZeRO/FSDP-like sharding.")
    ...
    # ✅ DDP: no device_map="auto"
    # ✅ attn_implementation to save memory/speed
    kw = dict(
        trust_remote_code=True,
        local_files_only=True,
        torch_dtype=dtype,
    ...
    model = Qwen2_5_VLForConditionalGeneration.from_pretrained(model_dir, **kw)
    ...
    # ✅ checkpointing is the #1 way to drop activation memory
    ...
    targs = TrainingArguments(
        ...
        deepspeed=args.deepspeed,  # ✅ 为空则不用；给路径则启用 ZeRO
    )
