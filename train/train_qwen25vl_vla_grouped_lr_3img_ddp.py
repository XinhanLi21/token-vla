...
"""
DDP Multi-GPU training script for Qwen2.5-VL-3B VLA
(3 images + prompt + state + past tokens -> future tokens)

Input:
  - images: front_left (left), zed_left (middle), front_right (right)
  - prompt: "Go straight" / "Turn left" / "Turn right" ...
  - vehicle state: speed/steer/heading (best-effort)
...
Multi-GPU:
  ...
  - Example (2 GPUs): CUDA_VISIBLE_DEVICES=0,1 torchrun --nproc_per_node=2 train_xxx.py ...
...
class DataCollatorQwenVLA3ImgTokens:
    ...
    # your mapping
    img_left_key: str = "front_left"   # left
    img_mid_key: str = "zed_left"      # middle
    img_right_key: str = "front_right" # right
    ...
    def __call__(self, batch: List[Dict[str, Any]]) -> Dict[str, torch.Tensor]:
        ...
        for ex in batch:
            ...
            # IMPORTANT: keep order consistent with textual statement (middle/left/right)
            images_batch.append([img_mid, img_left, img_right])
            ...
            state_parts = []
            if v is not None:
                state_parts.append(f"speed={v:.3f}m/s")
...
# -------------------------
# Trainer: grouped LR + optional GC
# -------------------------
def _unwrap_model(m):
...
class GroupedLRTrainer(Trainer):
    ...
    def create_optimizer(self):
        ...
        in_emb = base_model.get_input_embeddings()
        ...
        for n, p in base_model.named_parameters():
            ...
            is_traj = (p.data_ptr() in traj_param_ptrs)
            lname = n.lower()
    ...
    # transformers 新版 training_step: (model, inputs, num_items_in_batch)
...
def main():
    ...
    # DDP unused params
    ap.add_argument("--ddp_find_unused_parameters", type=str, default="auto",
                    choices=["auto", "true", "false"],
                    help="DDP find_unused_parameters. auto: true for traj_embed_head_only else false.")
    ...
    # ddp find unused
    if args.ddp_find_unused_parameters == "auto":
        ddp_find_unused = True if args.train_mode == "traj_embed_head_only" else False
    else:
        ddp_find_unused = (args.ddp_find_unused_parameters == "true")
    ...
    # NOTE: bf16 on RTX 50xx should be OK
    ...
    targs = TrainingArguments(
        ...
        learning_rate=args.lr_base,  # kept, but optimizer is grouped
    ...
    # only save on main process (Trainer handles this under DDP)
